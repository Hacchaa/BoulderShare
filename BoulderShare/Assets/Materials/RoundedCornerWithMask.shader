Shader "Custom/2D/RoundedCornerWithMask" {
Properties {
	_MainTex ("Texture", 2D) = "white" {}
	_Radius ("Radius px", Float) = 10
	_Width ("Width px", Float) = 100
	_Height ("Height px", Float) = 100
	_TexWidth ("Texture Width px", Float) = 100
	_TexHeight ("Texture Height px", Float) = 100
...
SubShader {
	...
	Pass {
		...
		float MaskInRect(float2 pixel, float2 halfRes){
			float2 firstQuadrant = abs(pixel);
			float alphaX = 1.0 - floor(firstQuadrant.x / halfRes.x);
			alphaX = max(alphaX, 0.0);
			float alphaY = 1.0 - floor(firstQuadrant.y / halfRes.y);
			alphaY = max(alphaY, 0.0);
