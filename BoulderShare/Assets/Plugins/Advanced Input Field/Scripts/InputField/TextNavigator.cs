...
namespace AdvancedInputFieldPlugin
{
	...
	public class TextNavigator
	{
		...
		private int richTextCaretPosition;
		...
		{
			get { return richTextCaretPosition; }
			internal set
			{
				SetRichTextCaretPosition(value, true);
			}
		}
		...
		public virtual int RichTextSelectionStartPosition
		{
			...
			set
			{
				SetRichTextSelectionStartPosition(value, true);
			}
		}
		...
		public virtual int VisibleSelectionStartPosition
		{
			...
			set
			{
				if(InputField.RichTextEditing)
				{
					RichTextSelectionStartPosition = value;
				}
				else if(InputField.LiveProcessing)
				{
					ProcessedSelectionStartPosition = value;
		...
		public virtual int RichTextSelectionEndPosition
		{
			...
			set
			{
				SetRichTextSelectionEndPosition(value, true);
			}
		}
		...
		public virtual int VisibleSelectionEndPosition
		{
			...
			set
			{
				if(InputField.RichTextEditing)
				{
					RichTextSelectionEndPosition = value;
				}
				else if(InputField.LiveProcessing)
				{
					ProcessedSelectionEndPosition = value;
		...
		internal virtual void SetRichTextCaretPosition(int richTextCaretPosition, bool richTextProcessAllowed = true)
		{
			if(this.richTextCaretPosition != richTextCaretPosition)
			{
				this.richTextCaretPosition = richTextCaretPosition;
			}
			UpdateCaret();

			if(InputField.RichTextEditing && richTextProcessAllowed)
			{
				TextEditFrame richTextEditFrame = new TextEditFrame(InputField.RichText, richTextCaretPosition, richTextSelectionStartPosition, richTextSelectionEndPosition);
				InputField.ApplyRichTextEditFrame(richTextEditFrame);
			}
		}
		...
		internal virtual void SetRichTextSelectionStartPosition(int richTextSelectionStartPosition, bool richTextProcessAllowed = true)
		{
			if(this.richTextSelectionStartPosition != richTextSelectionStartPosition)
			{
		...
		internal virtual void SetRichTextSelectionEndPosition(int richTextSelectionEndPosition, bool richTextProcessAllowed = true)
		{
			if(this.richTextSelectionEndPosition != richTextSelectionEndPosition)
			{
		...
		internal void RefreshRenderedText(bool forceRefresh = false)
		{
			...
			if(InputField.RichTextEditing)
			{
				InputField.RenderedText = InputField.RichText;
			}
			else if(InputField.LiveProcessing)
			{
				text = ProcessedText;
		...
		internal virtual void UpdateCaret(TextRenderer textRenderer)
		{
			...
			if(textRenderer.CharacterCount == 0 || textRenderer.Text.Length == 0) //Workaround to make sure the text generator will give a correct position for the first character
			{
				fixEmptyCaret = true;
		...
		internal virtual void UpdateSelectionArea(int currentPosition, int pressPosition, bool autoSelectWord)
		{
			if(currentPosition < pressPosition)
			{
				if(InputField.RichTextEditing)
				{
					RichTextSelectionStartPosition = currentPosition;
					if(autoSelectWord)
					{
						RichTextSelectionEndPosition = FindNextWordStart(pressPosition, Text);
					}
					else
					{
						RichTextSelectionEndPosition = pressPosition;
					}

					RichTextCaretPosition = currentPosition;
				}
				else if(InputField.LiveProcessing)
				{
				...
				else
				{
					...
					if(autoSelectWord)
					{
						SelectionEndPosition = FindNextWordStart(pressPosition, InputField.RichText);
					}
					else
					{
			...
			else if(currentPosition > pressPosition)
			{
				if(InputField.RichTextEditing)
				{
					if(autoSelectWord)
					{
						RichTextSelectionStartPosition = FindPreviousWordStart(pressPosition, InputField.RichText);
					}
					else
					{
						RichTextSelectionStartPosition = pressPosition;
					}

					RichTextSelectionEndPosition = currentPosition;
					RichTextCaretPosition = currentPosition;
				}
				else if(InputField.LiveProcessing)
				{
