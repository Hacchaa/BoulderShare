	...
	public class StandaloneTextNavigator: TextNavigator
	{
		...
		internal void MoveLeft()
		{
			...
			else
			{
				...
				{
					int resultIndex = InputField.RichTextProcessor.GetRichTextPrevious();
					if(resultIndex != -1)
					{
						RichTextCaretPosition = Mathf.Max(0, resultIndex);
					}
				}
				else
				{
		...
		internal void MoveRight()
		{
			...
			else
			{
				if(InputField.RichTextEditing)
				{
					int resultIndex = InputField.RichTextProcessor.GetRichTextNext();
					if(resultIndex != -1)
					{
						RichTextCaretPosition = Mathf.Min(InputField.RichText.Length, resultIndex);
					}
				}
				else
				{
