...
namespace BoulderNotes{
    public delegate void OnLoadTextureDel(Texture2D tex);
...
public class PickImageManager : SingletonMonoBehaviour<PickImageManager>
{
    ...
    public void OpenMediaActiveSheet(OnLoadTextureDel del, string title, Action OnSelecteDelete = null){
		...
		if (Application.platform != RuntimePlatform.IPhonePlayer){
            ...
            if (OnSelecteDelete != null){
                ISN_UIAlertAction deleteAction = new ISN_UIAlertAction("削除する", ISN_UIAlertActionStyle.Destructive, OnSelecteDelete);
	...
	private void OpenPhotoLibrary(){
        ...
        picker.Present((result) => {
            if (result.IsSucceeded) {
                ...
                if (loadTextureDel != null){
                    loadTextureDel(result.Image);
                }
            } else {
